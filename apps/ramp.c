...
#else /* defined(SYSV) */
#include <sys/resource.h>	/* defined(SYSV) */
#endif /* defined(SYSV) */
...
static char CVSID[] = "$RCSfile: ramp.c,v $ -- $Revision: 1.12 $\n";
#endif
...
int verbose = 0;		/* 0=print basic info, 1=print cpu rate, proc
				 * resource usage. */
int rec_boundary = 0;		/* 0=no record boundary,1 = record boundary */
     ...
     char **argv;
{
  ...
  while ((c = getopt (argc, argv,
		      "cderstuvBCDEIMPSTZb:f:L:l:n:p:q:A:F:G:O:R:m:a:")) !=
	 -1)
    {
      switch (c)
	{
...
#define MAX_WRITE_SIZE  1432

	  /*  Durst - modify to increase write size from 1 to buflen by 1 */
	  for (cnt = 1; nbytes < nbuf * buflen; nbytes += (cnt %
							   MAX_WRITE_SIZE),
	       cnt++)
	    {
	      if ((cc = Nwrite (fd, buf, (cnt % MAX_WRITE_SIZE))) != (cnt % MAX_WRITE_SIZE))
		{		/* Short write */
		  printf ("Expected to write %d, write returned %d\n",
			  (cnt % MAX_WRITE_SIZE), cc);
		}
	    }
#ifdef NOT_DEFINED

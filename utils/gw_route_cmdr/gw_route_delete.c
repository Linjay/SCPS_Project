...
extern int route_id;
gateway_command_t route_delete_cmd;


void
build_route_delete_cmd ()

{
	memset (&route_delete_cmd, 0x00, sizeof (route_delete_cmd));
	route_delete_cmd.command = GW_COMMAND_ROUTE_DELETE;
	route_delete_cmd.seq_num = 0x1;
	
	if (route_id >= 0) {
		route_delete_cmd.data.route_del.route_id = route_id;
	}
...
void
send_route_delete_cmd ()

{
	int  rc;

	rc = send (s_gw_route_cmdr, &route_delete_cmd, sizeof (route_delete_cmd) , 0);

	if (rc <= 0) {
		perror ("Error in sending route delete to gateway");
	} else {  
	}

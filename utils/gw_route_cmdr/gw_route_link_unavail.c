...
extern int route_id;
gateway_command_t route_unavail_cmd;


void
build_route_unavail_cmd ()

{
	memset (&route_unavail_cmd, 0x00, sizeof (route_unavail_cmd));
	route_unavail_cmd.command = GW_COMMAND_ROUTE_UNAVAIL;
	route_unavail_cmd.seq_num = 0x1;
	
	if (route_id >= 0) {
		route_unavail_cmd.data.route_del.route_id = route_id;
	}
	
}
	
void
verify_route_unavailable ()
...
void
send_route_unavail_cmd ()

{
	int  rc;

	rc = send (s_gw_route_cmdr, &route_unavail_cmd, sizeof (route_unavail_cmd) , 0);

	if (rc <= 0) {
		perror ("Error in sending route unavailable to gateway");
	} else {  
	}

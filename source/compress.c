...
static char CVSID[] = "$RCSfile: compress.c,v $ -- $Revision: 1.11 $\n";
#endif
...
tp_Compress (tp_Socket * socket, struct mbuff *m, u_char * chp)
{
  ...
  if ((opt_len = (tp_len - TP_NO_OPTS)))
    {
       ...
       *  - Timestamps: set bit in compressed hdr, copy timevals 
       *    only
       *  - All others:  copy options without mods
       */
      op1 = (char *) th + 20;	/* point to options */
      cnt = opt_len;
      for (; cnt > 0; cnt -= olen, op1 += olen)
	{
	  ...
	  switch (opt)
	    ...
	    case TPOPT_TIMESTAMP:
	      ...
	      opt_len -= olen;
	      changes |= TIME;
	      /* stick timestamps with the rest of the changes */
	      memcpy (cp, (op1 + 2), 8);
	      cp += 8;
	      continue;
	    }
  ...
  if (changes & 0xff)
    changes |= MORE;
  chl = ((changes & MORE) ? 5 : 4) + deltaS +
    ((changes & OPTS) ? (1 + opt_len) : 0);
  if (chl & 0x1)
    {
      /* 
  ...
  *cp++ = (u_char) (changes >> 8);
  ...
  socket->ph.length = htons (chl + m->m_ext.len);
  deltaA = checksum ((word *) chp, chl - 2) + m->m_ext.checksum;
  deltaA = ((deltaA >> 16) & 0xffff) + (deltaA & 0xffff);
  socket->ph.checksum = ((deltaA >> 16) & 0xffff) + (deltaA & 0xffff);
